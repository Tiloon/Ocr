        ...
        guchar g, guchar b)
{
    ...
    {
        if((i > ht) || (i * rowstride + rect->x * bpp >= ht * rowstride))
            break;
        //printf("y : %i, x : %i\n", i,  rect.w );
        ...
        if(i * rowstride + (rect->w + rect->x) * bpp > ht * rowstride)
            break;
